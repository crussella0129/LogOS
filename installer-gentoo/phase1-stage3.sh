...
if [[ -f /mnt/tmp/logos-uuids ]]; then
  ...
  die "No UUID file found at /mnt/tmp/logos-uuids — run phase0-partition.sh first"
fi

# Validate critical UUIDs are set
[[ -n "${BTRFS_UUID:-}" ]] || die "BTRFS_UUID not set — UUID detection failed"
[[ -n "${BOOT_UUID:-}" ]]  || die "BOOT_UUID not set — UUID detection failed"
[[ -n "${EFI_UUID:-}" ]]   || die "EFI_UUID not set — UUID detection failed"
...
UUID=${BTRFS_UUID}  /var/log          btrfs   subvol=@log,noatime,nodatacow                                    0      0

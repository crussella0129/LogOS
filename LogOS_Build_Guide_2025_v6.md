...
1. **Triple-Kernel Architecture**: Three kernels (linux-hardened, linux, linux-zen) provide security/performance flexibility through the Ringed City boot profiles
...
|---------|--------|-------------|----------|-------------------|
| **Gael** | linux-hardened | All enabled, SMT disabled, lockdown mode | Maximum threat environment | High (~15-30%) |
...
# /mnt/boot/vmlinuz-linux-hardened
...
```bash
# Read UUIDs again (in case of new shell)
CRYPT_UUID=$(cat /tmp/crypt_uuid)
BTRFS_UUID=$(cat /tmp/btrfs_uuid)
...
# Mitigations: All enabled, SMT disabled
# Use: Maximum threat environment
# --------------------------------------------
menuentry "LogOS - Gael Profile (Maximum Security)" --class arch --class gnu-linux --class gnu --class os {
    load_video
    set gfxpayload=keep
    insmod gzio
    insmod part_gpt
    ...
    echo 'Loading Linux Hardened with Full Mitigations...'
    linux /boot/vmlinuz-linux-hardened cryptdevice=UUID=$CRYPT_UUID:cryptroot root=/dev/mapper/cryptroot rootflags=subvol=@ rw audit=1 apparmor=1 lsm=landlock,lockdown,yama,integrity,apparmor,bpf mitigations=auto,nosmt lockdown=confidentiality quiet
...
# Mitigations: Auto (kernel default)
# Use: Daily secure operations
# --------------------------------------------
menuentry "LogOS - Midir Profile (Daily Driver)" --class arch --class gnu-linux --class gnu --class os {
    load_video
    set gfxpayload=keep
    insmod gzio
    insmod part_gpt
    ...
    insmod luks2
    search --no-floppy --fs-uuid --set=root $BTRFS_UUID
    echo 'Loading Linux Zen with Balanced Security...'
    linux /boot/vmlinuz-linux-zen cryptdevice=UUID=$CRYPT_UUID:cryptroot root=/dev/mapper/cryptroot rootflags=subvol=@ rw audit=1 apparmor=1 lsm=landlock,lockdown,yama,integrity,apparmor,bpf mitigations=auto quiet
    echo 'Loading initial ramdisk...'
...
# Use: Gaming, media production, HPC
# --------------------------------------------
menuentry "LogOS - Halflight Profile (Performance)" --class arch --class gnu-linux --class gnu --class os {
    load_video
    set gfxpayload=keep
    insmod gzio
    insmod part_gpt
    ...
    insmod luks2
    search --no-floppy --fs-uuid --set=root $BTRFS_UUID
    echo 'Loading Linux Zen with Performance Focus...'
    linux /boot/vmlinuz-linux-zen cryptdevice=UUID=$CRYPT_UUID:cryptroot root=/dev/mapper/cryptroot rootflags=subvol=@ rw audit=1 apparmor=1 lsm=landlock,lockdown,yama,integrity,apparmor,bpf mitigations=off quiet
    echo 'Loading initial ramdisk...'
    initrd /boot/initramfs-linux-zen.img
}

# --------------------------------------------
# MIDIR FALLBACK - Standard Kernel
# Kernel: linux (vanilla)
# Use: If linux-zen has issues
# --------------------------------------------
menuentry "LogOS - Midir Fallback (Standard Kernel)" --class arch --class gnu-linux --class gnu --class os {
    load_video
    set gfxpayload=keep
    insmod gzio
    insmod part_gpt
    ...
    insmod luks2
    search --no-floppy --fs-uuid --set=root $BTRFS_UUID
    echo 'Loading Standard Linux Kernel...'
    linux /boot/vmlinuz-linux cryptdevice=UUID=$CRYPT_UUID:cryptroot root=/dev/mapper/cryptroot rootflags=subvol=@ rw audit=1 apparmor=1 lsm=landlock,lockdown,yama,integrity,apparmor,bpf mitigations=auto quiet
    echo 'Loading initial ramdisk...'
...
# Now we need to substitute the actual UUIDs into the script
# This is the FIX for the dynamic UUID issue
sed -i "s/\$BTRFS_UUID/$BTRFS_UUID/g" /etc/grub.d/41_logos_profiles
sed -i "s/\$CRYPT_UUID/$CRYPT_UUID/g" /etc/grub.d/41_logos_profiles
```
...
check "Hardened kernel available" "ls /boot/vmlinuz-linux-hardened"
...
[ -f /boot/vmlinuz-linux-hardened ] && pass "Hardened kernel present" || fail "Hardened kernel missing"
...
pacman -S linux linux-zen linux-hardened
mkinitcpio -P
```
...
```
    ...
    │ linux-hardened│    │  linux-zen    │    │  linux-zen    │
